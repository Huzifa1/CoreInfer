...
def generate(method, model, tokenizer, ori_prompt, task_type, num_fewshot, num_tokens_to_generate, device, sampling_method, top_p):
    model.eval()
    if method in ['stable_guided', 'static_cut', 'dynamic_cut', 'dense', 'dynamic_cut_ci']:
...
def main(method, model_name, checkpoint_path, sparsity, start_num, end_num, token_sparsity, prompt, memory_limit, num_fewshot, task_type, num_tokens_to_generate, device, sampling_method, cluster_path = None, cpu_only = False, top_p = None, sparsity_levels_path = None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, choices=['stable_guided', 'similarity_guided', 'dynamic_cut', 'dense', 'static_cut', 'moving_cut', 'sparsity_levels'], default='stable_guided', help='Method to use (default: stable_guided).')

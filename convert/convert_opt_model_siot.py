...
class ReduceLayer(nn.Module):
    def __init__(self, weight, bias, sparsity, token_sparsity, start_num, end_num, memory_limit, cpu_only, num, original_neurons_num, siot_method_config):
...
def convert_opt_model_siot(model, sparsity, start_num, end_num, token_sparsity, memory_limit, cpu_only, original_neurons_num, siot_method_config):
    ...
    for name, module in tqdm(model.named_modules(), desc="Convert Opt Models"):
        if "fc1" in name or "fc2" in name:
            ...
            if num>start_num and num<end_num:
                ...
                if "fc1" in name:
                    NewLayer = ReduceLayer(module.weight, module.bias, sparsity, token_sparsity, start_num, end_num,memory_limit, cpu_only, num, original_neurons_num, siot_method_config)
                else:

...
class ReduceLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(0)>1:
            ...
            indices_all = torch.arange(0, true_value.shape[1]).cpu()
            ...
            self.filtered_W = self.weight[indices_all,:].cpu().to(device).to(torch.float16)
            self.filtered_bias = self.bias[indices_all].cpu().to(device).to(torch.float16)

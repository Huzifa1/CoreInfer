...
def split_state_dict(state_dict, max_size_mb: int, base_path, prefix="pytorch_model-split", start_index=0):
    ...
    for key, value in state_dict.items():
        ...
        if current_size + tensor_bytes > max_size_bytes and current_split:
            split_file = base_path / f"{prefix}-{part_num:05d}.bin"
            torch.save(current_split, split_file)
...
def update_index_file(index_path, new_mapping, new_bin_files):
    with open(index_path, "r") as f:
        index_data = json.load(f)

    index_data["weight_map"] = new_mapping
    index_data["metadata"]["total_size"] = sum(Path(index_path.parent / fname).stat().st_size for fname in new_bin_files)
...
def main(model_dir, max_size_mb):
    ...
    new_bin_files = sorted(set(full_mapping.values()))
    update_index_file(index_file, full_mapping, new_bin_files)
